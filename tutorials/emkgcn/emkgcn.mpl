...
# Here we implement O(h^2) Cracnk-Nickelson Scheme 
# for time evolution of fields and 
# O(h^2) Mid-point FDA to solve Einstein's equation
# (ODEs) The outer boudary is outgoing and inner boundary
# is not a real boundary but just imposing the behaviour
# of the fields at Phi(r-->0) 
# Also demonstraiting of defining non-uniform FDA operator
#-----------------------------------------------------
...
# CodeGeneration of Maple will change the name of the variable
#----------------------------------------------------------------
...
#-------------------------
DISS := f -> -epsdis/(16*ht)*( 6*FD(f,[[0],[0]])  + FD(f,[[0],[2]]) + FD(f,[[0],[-2]] )
                             - 4*( FD(f,[[0],[1]] ) +  FD(f,[[0],[-1]] ) ) 
                             );

# Non-uniform derivative: in x (df/d(x^3))
DCR3 := f-> ( FD(f,[[0],[1]]) - FD(f,[[0],[-1]]) ) / ( x(i+1)^3 - x(i-1)^3 );

QFIT := f-> FD(f,[[0],[0]]) -4*FD(f,[[0],[1]])/3 + FD(f,[[0],[2]])/3;
#------------------------------------------------------
...
Gen_Res_Code(eq_pp,input="c",proc_name="ire_pp");
Gen_Res_Code(eq_pi,input="c",proc_name="ire_pi");

Gen_Res_Code(lhs(eq_a)-rhs(eq_a),input="c",proc_name="ire_a");
Gen_Res_Code(lhs(eq_alpha)-rhs(eq_alpha),input="c",proc_name="ire_alpha");
...
spec_init_pp := [
						{ i =[1,1,1]    }  = myzero*x ,
						{ i =[2,Nx-1,1] }  = diff(phi(t,x),x) ,
						{ i =[Nx,Nx,1]  }  = myzero*x
                ];

spec_init_pi := [
                  { i =[1,1,1]    }  = myzero*x ,
						{ i =[2,Nx-1,1] }  = idsignum*(diff(phi(t,x),x)+phi(t,x)/x),
						{ i =[Nx,Nx,1]  }  = myzero*x
                ];
...
# Do not used upper case for proc 
# names because of C/FORTRAN U/L case Sens/Insensitivity
...
# (No need for higher derivatives at the moment)
FD_table[t] := [ [0] , [0,1] ];
...
spec_evolve_pp := [
                ...
                { i = [3,Nx-2,1] }    = eq_pp_L_d - AVGT(eq_pp_R_d + DISS(pp(n,i))),
                { i = [Nx-1,Nx-1,1] } = eq_pp_L_d - AVGT(eq_pp_R_d),
                { i = [Nx,Nx,1] }     = eq_pp_L_d - AVGT(eq_pp_ob_R_d)
                  ];

spec_evolve_pi := [
  { i = [1,1,1] } = AVGT(QFIT(pi(n,i))),
  { i = [2,2,1] } = eq_pi_L_d - AVGT(3*DCR3(x(i)^2*alpha(i)*pp(n,i)/a(i))) ,
  { i = [3,Nx-2,1] } = eq_pi_L_d-AVGT(3*DCR3(x(i)^2*alpha(i)*pp(n,i)/a(i))+DISS(pi(n,i))),
  { i = [Nx-1,Nx-1,1] } = eq_pi_L_d - AVGT(3*DCR3(x(i)^2*alpha(i)*pp(n,i)/a(i))) ,
#{ i = [2,2,1] }= eq_pi_L_d - AVGT(eq_pi_R_d) ,
#{ i = [3,Nx-2,1] }    = eq_pi_L_d - AVGT(eq_pi_R_d + DISS(pi(n,i))),
#{ i = [Nx-1,Nx-1,1] } = eq_pi_L_d - AVGT(eq_pi_R_d),
  { i = [Nx,Nx,1] }     = eq_pi_L_d - AVGT(eq_pi_ob_R_d)
                   ];

# For non-uniform derivative 
#  { i = [2,2,1] } = eq_pi_L_d - AVGT(3*DCR3(x(i)^2*alpha(i)*pp(n,i)/a(i))) ,
#  { i = [3,Nx-2,1] } = eq_pi_L_d-AVGT(3*DCR3(x(i)^2*alpha(i)*pp(n,i)/a(i))+DISS(pi(n,i))),
#  { i = [Nx-1,Nx-1,1] } = eq_pi_L_d - AVGT(3*DCR3(x(i)^2*alpha(i)*pp(n,i)/a(i))) ,
...
A_Gen_Res_Code(spec_evolve_pp,input="d",proc_name="res_pp");
A_Gen_Res_Code(spec_evolve_pi,input="d",proc_name="res_pi");
...
A_Gen_Solve_Code(spec_evolve_pp,{pp(n+1,i)},input="d",proc_name="u_pp");
A_Gen_Solve_Code(spec_evolve_pi,{pi(n+1,i)},input="d",proc_name="u_pi");


#-------------------------------------------------------------------
# Solving the ODEs using a second order implicit FD scheme
#-------------------------------------------------------------------

#Forward integrationf for a
FD_table[x] := [ [0], [0,1] ]:

eq_lna_R_d := Gen_Sten(x*rhs(eq_lna)):
eq_lna_L_d := Gen_Sten(lhs(eq_lna)):


# Will integrate alpha backward:
FD_table[x] := [ [0] ,[-1,0] ]:
eq_lnalpha_R_d := Gen_Sten(x*rhs(eq_lnalpha)):
eq_lnalpha_L_d := Gen_Sten(lhs(eq_lnalpha)):

# This will be second order scheme for mid point:
ODE_1 := eq_lna_L_d - AVGX(eq_lna_R_d)/AVGX(x(i));
ODE_2 := eq_lnalpha_L_d - BAVGX(eq_lnalpha_R_d)/BAVGX(x(i));

ODE_1:= eval(ODE_1,{lna(i+1)=lnap1}):
ODE_2:= eval(ODE_2,{lnalpha(i-1)=lnalpham1}):

ODE_Solver_1 :=Newton_Solver([ODE_1],[lnap1]):
ODE_Solver_2 :=Newton_Solver([ODE_2],[lnalpham1]):

ODE_1 := eval(ODE_1,{lnap1=lna(i+1)}):
ODE_2 := eval(ODE_2,{lnalpham1=lnalpha(i-1)}):

ODE_Solver_1 := eval(ODE_Solver_1,{lnap1=lna(i+1)});
ODE_Solver_2 := eval(ODE_Solver_2,{lnalpham1=lnalpha(i-1)});
...
ODE_Solver_1:=evalf(ODE_Solver_1);
tss:=CodeGeneration[Fortran](ODE_Solver_1,resultname="ap1",output=string,limitvariablelength=false,optimize=opt,defaulttype=float);
fname:="calc_a.inc_";
fp:=fopen(fname,WRITE);
fprintf(fp,tss);
fclose(fp);
...
ODE_Solver_2:=evalf(ODE_Solver_2);
tss:=CodeGeneration[Fortran](ODE_Solver_2,resultname="alpham1",output=string,limitvariablelength=false,optimize=opt,defaulttype=float);
...
all_exp:=a(x)+alpha(x)+myzero+phi(t,x)+Ttt(x)+Trr(x)+eq_pp+eq_pi+phi_init+idsignum+epsdis;
OH:={"init_phi","init_pp","init_pi","calc_ttt","calc_trr",
     "ire_pp","ire_pi",
     "u_pp","u_pi","res_pp","res_pi"};
